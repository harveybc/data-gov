"""Merge plugin params → defaults → file config → long-form CLI flags."""
...
def merge_config(defaults, plugin_param_dicts, file_config, cli_args, unknown_args):
    """
    Precedence (later wins), same house rule as predictor / doin-node:

    1. plugin_params of loaded plugins
    2. app.config DEFAULT_VALUES
    3. JSON from --load_config
    4. long-form CLI ``--flags`` only
    """
    merged = {}
